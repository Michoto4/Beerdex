  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      values = await Object.assign(values, {
        beerVerticalStyle: sliderValueVertical,
      });
      values = await Object.assign(values, {
        beerHorizontalStyle: sliderValueHorizontal,
      });
      values = await Object.assign(values, {
        beerWidthStyle: sliderValueWidth,
      });
    ...
    <div className={styles.popupBackground}>
      ...
      >
        ...
        <div className={styles.middlePart}>
          ...
          <div className={styles.beerPhotoContainer}>
            <label htmlFor="beerPhoto">
              <img
                style={{
                  bottom: `${sliderValueVertical}%`,
                  left: `${sliderValueHorizontal}%`,
                  width: `${sliderValueWidth}px`,
                }}
                src={file || addBeerDefault}
                alt="Beer Photo"
              />
            </label>
          ...
          <div className={styles.beerImageSliderContainerVertical}>
            <input
              onChange={sliderUpdateVertical}
              type="range"
              min="-200"
              max="200"
              className={styles.sliderInputVertical}
            />
          </div>
          {/* BEER IMAGE POSITION HORIZONTAL */}
          <div className={styles.beerImageSliderContainerHorizontal}>
            <input
              onChange={sliderUpdateHorizontal}
              type="range"
              min="-200"
              max="200"
              className={styles.sliderInputHorizontal}
            />
            <input
              onChange={sliderUpdateWidth}
              type="range"
              min="100"
              max="500"
              className={styles.sliderInputWidth}
            />
          </div>

          {/* BEER RATING */}
          <div className={styles.beerRatingContainer}>
            <label htmlFor="beerRating">{t("beerRating")}</label>
            <br />
            <input
        ...
        <div className={styles.bottomPart}>
          <button
            type="button"
            className={styles.beerCancelButton}
            onClick={() => props.setTrigger(false)}
          >
            {t("cancel")}
          </button>
          <button
            ...
            className={styles.beerCreateButton}
          >
            {t("create")}
          </button>
        </div>
